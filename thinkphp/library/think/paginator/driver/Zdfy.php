...
class Zdfy extends Paginator
{
    ...
    protected  function totalshow()
    {
        ...
        $totalhtml="<li class=\"disabled\"><span>共".$this->total."条";
        return $totalhtml;
    ...
    protected  function gopage(){
        ...
        $str = '<li><select class="input-sm w100" id="zdfy">';
        for($i=1 ; $i <= $all ; $i++){
            $url = $this->url($i);
            $select = ($cpage == $i) ? "selected" : "";
            $str .= '<option value="'.$i.'" data ="'.$url.'" '.$select.' >'.$i.'/'.$all.'页</option>';
        }
        $str .= '</select></li>';

        return $str;
    }
    protected  function gopagejs(){
        $str = '<script>';
        $str .= '$(function(){'."\r\n";
        $str .= '$("#zdfy").change(function(){'."\r\n";
        $str .= 'var durl = $("#zdfy option:selected").attr("data");'."\r\n";
        // $str .= 'alert(durl);'."\r\n";
        $str .= 'window.location.href=durl;'."\r\n";
        $str .= '});'."\r\n";
        $str .= '})';
        $str .= '</script>';
        return $str;
    ...
    public function render()
    {
        if ($this->hasPages()) {
            if ($this->simple) {
                ...
                return sprintf(
                    '<ul class="pagination"> %s %s %s %s %s %s %s </ul> %s ',
                    //显示数量页码信息
                    $this->totalshow(),
                    //第一页
                    $this->showfirstpage(),
